package io.github.vdaburon.jmeter.otelxml;
...
public class OtelJMeterManager {
	private static final Logger LOGGER = LoggerFactory.getLogger(OtelJMeterManager.class);
	// fixed string to find in the jmeter script or in the extracted xml files
	private static final String PARAM_TC_NAME = "@@TC_NAME";
	private static final String COMMENT_BEGIN_OTEL_APM = "@@OTEL_APM_BEGIN";
	private static final String COMMENT_END_OTEL_APM = "@@OTEL_APM_END";
	private static final String COMMENT_OTEL_UDV = "@@OTEL_APM_UDV";
	private static final String COMMENT_SETUP_INIT_TRACER = "@@OTEL_SETUP_INIT_TRACER";
	...
	public static final String EXTRACT_START_JSR223 = "otel_extract_start_transaction_ignore_jsr223.xml";
	public static final String EXTRACT_END_JSR223 = "otel_extract_end_transaction_ignore_jsr223.xml";
	public static final String EXTRACT_UDV_ELASTIC = "otel_extract_udv_elastic_under_testplan.xml";
	...
	public static void main(String[] args) {
		...
		modifyAddSamplerForOtelApm(sJMeterXmlFileScript, sJMeterXmlFileScriptOutModif, action, regex, sExtractAddStartJsr, sExtractAddEndJsr, sExtractUdvUnderTp);
	...
	public static void modifyAddSamplerForOtelApm(String jmeterXmlScript, String sJMeterXmlFileScriptModif, String action, String sRegexTcLabel, String sExtractAddStartJsr, String sExtractAddEndJsr, String sExtractUdvUnderTp) {
	...
	private static LinkedList<String> removeSamplerForElasticApm(LinkedList<String> lkfileJMeterOrig, String regexTc, LinkedList<String> lkStart, LinkedList<String> lkEnd, LinkedList<String> lkUdvUnderTp) {
		...
		Pattern patternCommentStartEltTc = Pattern.compile(".*?<stringProp name=\"TestPlan.comments\">" + COMMENT_BEGIN_OTEL_APM + "</stringProp>"); // JSR223 for begin transaction
		Pattern patternStartEltJsr223 = Pattern.compile(".*?<JSR223Sampler guiclass=\"TestBeanGUI\" testclass=\"JSR223Sampler\".*");

		Pattern patternCommentEndEltTc = Pattern.compile(".*?<stringProp name=\"TestPlan.comments\">" + COMMENT_END_OTEL_APM + "</stringProp>"); // JSR223 for end transaction
		Pattern patternCommentEltArgument = Pattern.compile(".*?<stringProp name=\"TestPlan.comments\">" + COMMENT_OTEL_UDV + "</stringProp>"); // UDV with ELASTIC_APM_UDV
		Pattern patternStartEltArguments = Pattern.compile(".*?<Arguments guiclass=\"ArgumentsPanel\" testclass=\"Arguments\".*");

		Pattern patternCommentSetupThreadGroup = Pattern.compile(".*<stringProp name=\"TestPlan.comments\">" + COMMENT_SETUP_INIT_TRACER + "</stringProp>");
		Pattern patternSetupStartThreadGroup = Pattern.compile(".*?<SetupThreadGroup guiclass=\"SetupThreadGroupGui\" testclass=\"SetupThreadGroup\" .*? enabled=\"true\">");
		Pattern patternSetupEndThreadGroup = Pattern.compile(".*?</SetupThreadGroup>");

		Pattern patternInitEndGroovyCode = Pattern.compile(".*?</JSR223Sampler>");
		...
		while (it.hasNext()) {
			...
			if (isCommentStartEltTc || isCommentEndEltTc) { // @@OTEL_APM_BEGIN or @@OTEL_APM_END
				// remove lines in the lkReturn (rewind) from comment  @@OTEL_APM_BEGIN to <JSR223Sampler
			...
			if (isCommentEltArgument) {
				// remove lines in the lkReturn (rewind) from comment  @@OTEL_APM_UDV to <Arguments
				boolean bNeedRemove = true;
				while (bNeedRemove) {
					String lastLine = lkReturn.removeLast();
			...
			Matcher matcherCommentSetupThreadGroup = patternCommentSetupThreadGroup.matcher(currentLine);
			boolean isCommentSetupThreadGroup = matcherCommentSetupThreadGroup.matches();

			if (isCommentSetupThreadGroup) {
				// remove lines in the lkReturn (rewind) from comment  @@OTEL_SETUP_INIT_TRACER to <SetupThreadGroup
				boolean bNeedRemove = true;
				while (bNeedRemove) {
					String lastLine = lkReturn.removeLast();

					Matcher matcherSetupStartThreadGroup = patternSetupStartThreadGroup.matcher(lastLine);
					boolean isSetupThreadGroup = matcherSetupStartThreadGroup.matches();
					if (isSetupThreadGroup) {
						bNeedRemove = false;
					}

				}
				// remove lines in reading file (forward) from comment  @@OTEL_SETUP_INIT_TRACER to </SetupThreadGroup
				bNeedRemove = true;
				while (bNeedRemove) {
					String tempLine = it.next(); //
					Matcher matcherSetupEndThreadGroup = patternSetupEndThreadGroup.matcher(tempLine);
					boolean isSetupEndThreadGroup = matcherSetupEndThreadGroup.matches();
					if (isSetupEndThreadGroup) {
						bNeedRemove = false;
					}
				}
				String tempLine2 = it.next(); // read <hashTree>

				// remove lines in reading file (forward) from <hashTree> to </JSR223Sampler>
				bNeedRemove = true;
				while (bNeedRemove) {
					String tempLine = it.next(); //
					Matcher matcherInitEndGroovyCode = patternInitEndGroovyCode.matcher(tempLine);
					boolean isInitEndGroovyCode = matcherInitEndGroovyCode.matches();
					if (isInitEndGroovyCode) {
						bNeedRemove = false;
					}
				}
				String tempLine3 = it.next(); // read <hashTree/>
				bAlreadyAdd = true;
			}
	...
	private static Options createOptions() {
		...
		Option extractUdvOpt = Option.builder(K_EXTRACT_UDV_OPT).argName(K_EXTRACT_UDV_OPT).hasArg(true)
				.required(false)
				.desc("optional, file contains User Defined Variables and setUp Thread Group for init OpenTelemetry (e.g : extract_udv.xml), default read file in the jar")
				.build();
	...
	private static void helpUsage(Options options) {
		...
		String footer = "E.g : java -jar jmeter-otel-apm-<version>-jar-with-dependencies.jar -" + K_JMETER_FILE_IN_OPT + " script1.jmx -"
				+ K_JMETER_FILE_OUT_OPT + " script1_add.jmx -" + K_ACTION_OPT + " ADD -"
				+ K_REGEX_OPT + " SC.*\n";
		footer+="E.g : java -jar jmeter-otel-apm-<version>-jar-with-dependencies.jar -" + K_JMETER_FILE_IN_OPT + " script1_add.jmx -"
				+ K_JMETER_FILE_OUT_OPT + " script1_remove.jmx -" + K_ACTION_OPT + " REMOVE -"
				+ K_REGEX_OPT + " .*";
		formatter.printHelp(120, OtelJMeterManager.class.getName(),
				OtelJMeterManager.class.getName(), options, footer, true);
	}
}
